...
Namespace Monitor.Core.Utilities
	...
	''' </summary>
	Public NotInheritable Class JunctionPoint
		Private Sub New()
		End Sub
		''' <summary>
		...
		Private Structure REPARSE_DATA_BUFFER
			...
			''' </summary>
			<MarshalAs(UnmanagedType.ByValArray, SizeConst := &H3ff0)> _
			Public PathBuffer As Byte()
		End Structure

		<DllImport("kernel32.dll", CharSet := CharSet.Auto, SetLastError := True)> _
		Private Shared Function DeviceIoControl(hDevice As IntPtr, dwIoControlCode As UInteger, InBuffer As IntPtr, nInBufferSize As Integer, OutBuffer As IntPtr, nOutBufferSize As Integer, _
			ByRef pBytesReturned As Integer, lpOverlapped As IntPtr) As Boolean
		End Function

		<DllImport("kernel32.dll", SetLastError := True)> _
		Private Shared Function CreateFile(lpFileName As String, dwDesiredAccess As EFileAccess, dwShareMode As EFileShare, lpSecurityAttributes As IntPtr, dwCreationDisposition As ECreationDisposition, dwFlagsAndAttributes As EFileAttributes, _
		...
		Public Shared Sub Create(junctionPoint As String, targetDir As String, overwrite As Boolean)
			...
			Using handle As SafeFileHandle = OpenReparsePoint(junctionPoint, EFileAccess.GenericWrite)
				...
				Try
					...
					Dim result As Boolean = DeviceIoControl(handle.DangerousGetHandle(), FSCTL_SET_REPARSE_POINT, inBuffer, targetDirBytes.Length + 20, IntPtr.Zero, 0, _
		...
		''' </remarks>
		''' <param name="junctionPoint">The junction point path</param>
		Public Shared Sub Delete(junctionPoint As String)
			...
			Using handle As SafeFileHandle = OpenReparsePoint(junctionPoint, EFileAccess.GenericWrite)
				...
				Try
					...
					Dim bytesReturned As Integer
					Dim result As Boolean = DeviceIoControl(handle.DangerousGetHandle(), FSCTL_DELETE_REPARSE_POINT, inBuffer, 8, IntPtr.Zero, 0, _
						bytesReturned, IntPtr.Zero)
		...
		Public Shared Function GetTarget(junctionPoint As String) As String
		...
		Private Shared Function InternalGetTarget(handle As SafeFileHandle) As String
			...
			Try
				Dim bytesReturned As Integer
				Dim result As Boolean = DeviceIoControl(handle.DangerousGetHandle(), FSCTL_GET_REPARSE_POINT, IntPtr.Zero, 0, outBuffer, outBufferSize, _
					bytesReturned, IntPtr.Zero)
		...
		Private Shared Function OpenReparsePoint(reparsePoint As String, accessMode As EFileAccess) As SafeFileHandle
			Dim reparsePointHandle As New SafeFileHandle(CreateFile(reparsePoint, accessMode, EFileShare.Read Or EFileShare.Write Or EFileShare.Delete, IntPtr.Zero, ECreationDisposition.OpenExisting, EFileAttributes.BackupSemantics Or EFileAttributes.OpenReparsePoint, _
				IntPtr.Zero), True)
		...
		Private Shared Sub ThrowLastWin32Error(message As String)
